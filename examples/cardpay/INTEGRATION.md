# Интеграция с провайдером «CardPay Global Payouts API»

| | |
|---|---|
| Провайдер | `cardpay` |
| Спецификация | `cardpay.yaml`, версия 2.4.0, OpenAPI 3.0.3 |
| Класс сервиса | `Provider::CardpayService < Provider::BaseService` |
...
|---|---|---|---|
| `CARDPAY_BASE_URL` | базовый URL API провайдера (константа BASE_URL) | `https://api.sandbox.cardpay.example` — песочница из спецификации | нет |
| `CARDPAY_OPEN_TIMEOUT` | таймаут соединения, секунды (константа OPEN_TIMEOUT) | 5 | нет |
| `CARDPAY_READ_TIMEOUT` | таймаут чтения ответа, секунды (константа READ_TIMEOUT) | 15 | нет |

Продакшен: задайте `CARDPAY_BASE_URL=https://api.cardpay.example` — второй сервер спецификации.

Секреты через ENV не читаются: ключи credentials — `token`, `webhook_secret`.
...
|---|---|---|---|---|
| `check_conditions(operation, request_method)` | — (без обращения к провайдеру) | значение шлюза, передаётся в super: `create`, `status`, `check` | не применимо | `success` или `failure` с кодом платформы `:unprocessable_entity` и ключом условия: `errors.amount_below_minimum`, `errors.amount_above_maximum`, `errors.external_id_too_long` |
| `create_request(operation, request_method = 'create')` | `POST /v2/transfers` | `'create'` по умолчанию | заголовок `X-Request-Id`, ключ — UUID v5 от operation.id (пространство имён из rules/idempotency.yml); на повтор провайдер отвечает: 409 со схемой успешного ответа — сервис подхватывает существующую операцию | `success` с идентификатором операции у провайдера — платформа берёт его как `payload.dig(:result, :id)`; `failure` с кодом платформы по FAILURE_CODES |
| `process_callback(payload)` | `POST /webhooks/transfer-events` — входящий, вызывает провайдер | — | нет: входящий запрос; повторное уведомление применяет тот же статус ещё раз | `success` после `approve_operation` / `reject_operation`, без result; `failure` с кодом платформы при неверной подписи, неизвестной операции или событии |
| `fetch_status(operation)` | `GET /v2/transfers/{transfer_id}` | — | идемпотентен по семантике HTTP (RFC 9110, GET) | `success` после перевода статуса хелпером, без result; `failure` с кодом платформы и ключом `errors.operation_not_found`, `errors.status_unknown` или ошибкой провайдера |

Не отображено на контракт Provider::BaseService: отдельные публичные методы, платформа зовёт их сама; см. report.md.

| Метод | HTTP-метод и путь провайдера | request_method | Идемпотентность | Результат |
|---|---|---|---|---|
| `cancel(operation)` — роль cancel (эвристика 0.95) | `POST /v2/transfers/{transfer_id}/cancel` | — | нет | `success` или `failure` с кодом платформы; до запроса — проверка CANCELLABLE_STATUSES |
| `refund(operation)` — роль refund (эвристика 0.92) | `POST /v2/refunds` | — | нет | тело ответа (Hash) или `failure` с кодом платформы по FAILURE_CODES |
| `balance` — роль balance (эвристика 0.95) | `GET /v2/accounts/balance` | — | идемпотентен по семантике HTTP (RFC 9110, GET) | тело ответа (Hash) или `failure` с кодом платформы по FAILURE_CODES |
...
| `queued` | `in_progress` | синоним справочника rules/statuses.yml | 0.90 |
| `returned` | `rejected` | синоним справочника rules/statuses.yml | 0.90 |
| `sending` | `in_progress` | синоним справочника rules/statuses.yml | 0.90 |
| `succeeded` | `approved` | синоним справочника rules/statuses.yml | 0.90 |
...
|---|---|---|---|
| `transfer.declined` | `rejected` | синоним справочника rules/statuses.yml | 0.90 |
| `transfer.queued` | `in_progress` | синоним справочника rules/statuses.yml | 0.90 |
| `transfer.returned` | `rejected` | синоним справочника rules/statuses.yml | 0.90 |
| `transfer.succeeded` | `approved` | синоним справочника rules/statuses.yml | 0.90 |
...
|---|---|---|---|---|
| `account_closed` | любой — читается из тела ответа | только в примерах | `reject` | `failure(platform_failure_code(response.status, :reject), 'errors.account_closed')` |
| `card_declined` | любой — читается из тела ответа | enum и примеры | `reject` | `failure(platform_failure_code(response.status, :reject), 'errors.card_declined')` |
| `duplicate_request` | любой — читается из тела ответа | enum и примеры | `reject` | `failure(platform_failure_code(response.status, :reject), 'errors.duplicate_request')` |
| `expired_card` | любой — читается из тела ответа | enum | `reject` | `failure(platform_failure_code(response.status, :reject), 'errors.expired_card')` |
| `insufficient_funds` | любой — читается из тела ответа | enum и примеры | `retry_backoff` | `failure(platform_failure_code(response.status, :retry_backoff), 'errors.insufficient_funds')` |
| `internal_error` | любой — читается из тела ответа | enum и примеры | `retry_backoff` | `failure(platform_failure_code(response.status, :retry_backoff), 'errors.internal_error')` |
| `invalid_card_number` | любой — читается из тела ответа | enum и примеры | `reject` | `failure(platform_failure_code(response.status, :reject), 'errors.invalid_card_number')` |
| `issuer_unavailable` | любой — читается из тела ответа | enum | `retry_backoff` | `failure(platform_failure_code(response.status, :retry_backoff), 'errors.issuer_unavailable')` |
| `limit_exceeded` | любой — читается из тела ответа | enum | `reject` | `failure(platform_failure_code(response.status, :reject), 'errors.limit_exceeded')` |
| `rate_limited` | любой — читается из тела ответа | только в примерах | `retry_backoff` | `failure(platform_failure_code(response.status, :retry_backoff), 'errors.rate_limited')` |
| `transfer_not_cancelable` | любой — читается из тела ответа | только в примерах | `reject` | `failure(platform_failure_code(response.status, :reject), 'errors.transfer_not_cancelable')` |
| `transfer_not_found` | любой — читается из тела ответа | только в примерах | `reject` | `failure(platform_failure_code(response.status, :reject), 'errors.transfer_not_found')` |
...
Дедупликация. Ответ 409 у операции `createTransfer` со схемой успешного ответа — не ошибка, а успешный путь по draft-ietf-httpapi-idempotency-key-header: `create_request` подхватывает существующую операцию (константа DEDUP_STATUS). В ERROR_MAP этот случай не входит.
...
providers:
  cardpay:
    service: Provider::CardpayService
    base_url: https://api.sandbox.cardpay.example  # переменная окружения CARDPAY_BASE_URL, адрес из серверов спецификации
    credentials: [token, webhook_secret]
    request_methods: [create, status, check]
    currencies: [USD]  # константа CURRENCY сервиса: enum: [USD] у поля `currency`
    amount:
      platform_unit: major
      provider_unit: minor
      multiplier: 100  # ISO 4217: экспонента USD 2
      minimum: 5  # minimum: 500 в единицах провайдера = 5.00 USD в мажорных (задано явно 1.00)
      maximum: 50_000  # maximum: 5000000 в единицах провайдера = 50000.00 USD в мажорных (задано явно 1.00)
    fields:
      merchant_reference: { max_length: 40 }  # роль external_id
    webhook:
      path: "/webhooks/transfer-events"
...
|---|---|---|---|
| `amount_minor` | `amount` | нет | `operation.amount` |
| `currency` | `currency` | нет | `CURRENCY` |
| `merchant_reference` | `external_id` | нет | `operation.id` |
| `destination.method` | `recipient_type` | `card` | `'card'` |
| `destination.method` | `recipient_type` | `wallet` | `'wallet'` |
| `destination.pan` | `card_number` | `card` | `operation.payout_requisite['card_number']` |
| `destination.expiry_month` | роль не распознана | `card` | — заполните вручную |
| `destination.expiry_year` | роль не распознана | `card` | — заполните вручную |
| `destination.holder_name` | роль не распознана | `card` | — заполните вручную |
| `destination.holder_name` | роль не распознана | `wallet` | — заполните вручную |
| `destination.issuer_country` | роль не распознана | `card` | — заполните вручную |
| `destination.issuer_country` | роль не распознана | `wallet` | — заполните вручную |
| `destination.wallet_id` | роль не распознана | `wallet` | — заполните вручную |
| `statement_descriptor` | роль не распознана | нет | — заполните вручную |
| `metadata` | роль не распознана | нет | — заполните вручную |

## 7. Схема подписи вебхука

| Параметр | Значение | Источник |
|---|---|---|
| заголовок с подписью | `webhook-signature` | задано явно 1.00 |
| алгоритм | `hmac_sha256` | по справочнику 0.90 |
| кодировка | `base64` | по справочнику 0.90 |
| подписываемая строка | `id_timestamp_body` | по справочнику 0.90 |
| секрет | `provider.credentials[:webhook_secret]` | по справочнику 0.90 |
| допуск по времени, секунды | `300` | по справочнику 0.90 |
| префикс значения | `v1,` | профиль Standard Webhooks (rules/signatures.yml) |
...
- уведомление с меткой времени старше 300 с отклоняется (`failure(:unauthorized, 'errors.signature_expired')`) — защита от повтора
- значение заголовка — список версий через пробел с префиксом `v1,`; принимается совпадение любой версии
...
signed = [headers['webhook-id'], headers['webhook-timestamp'], raw_body].join('.')
expected = [OpenSSL::HMAC.digest('SHA256', secret, signed)].pack('m0')
given = headers['webhook-signature'].split.map { |v| v.delete_prefix('v1,') }
valid = OpenSSL.fixed_length_secure_compare(expected, given)
```

## 8. Как подключить в приложение

1. Положите `cardpay_service.rb` в каталог провайдеров платформы рядом с
   остальными наследниками `Provider::BaseService`; класс — `Provider::CardpayService`.
...
- условная обязательность `pan` при method = card прочитана как намёк в описании с уверенностью 0.50 (подробнее — report.md)
- обязательное поле `destination.expiry_month` без роли: в payload со значением `11` и TODO (ни имя, ни ограничения не совпали ни с одной ролью rules/roles.yml) (подробнее — report.md)
- условная обязательность `expiry_month` при method = card прочитана как намёк в описании с уверенностью 0.50 (подробнее — report.md)
- обязательное поле `destination.expiry_year` без роли: в payload со значением `2029` и TODO (ни имя, ни ограничения не совпали ни с одной ролью rules/roles.yml) (подробнее — report.md)
- условная обязательность `expiry_year` при method = card прочитана как намёк в описании с уверенностью 0.50 (подробнее — report.md)
- обязательное поле `destination.wallet_id` без роли: в payload со значением `'wlt_5KD2H8ZQ'` и TODO (ни имя, ни ограничения не совпали ни с одной ролью rules/roles.yml) (подробнее — report.md)
- условная обязательность `wallet_id` при method = wallet прочитана как намёк в описании с уверенностью 0.50 (подробнее — report.md)
- условие cancel_status_restriction прочитано из прозы: queued (эвристика 0.60) (подробнее — report.md)
