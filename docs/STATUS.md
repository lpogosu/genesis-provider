...
Обновлено: 6 сентября 2026. Контракт исправлен по письменным ответам
...
не плоские поля, которых у платформы нет. В каталоге десять спек: выданная,
три свои (`cardpay`, `depositbank`, `broken`) и шесть чужих настоящих (Adyen
Payout, Adyen Transfers, PayPal Payouts, Paystack, GOV.UK Pay, Moov); все
десять проходят `./integrate` без стектрейса одной командой. Покрытие
печатается двумя цифрами: от всех найденных элементов и в границах
контракта.
...
прямо. Измеренное покрытие (первая цифра — от всех найденных элементов,
вторая — в границах контракта): NovaPay 75 и 98 %, Paystack 53 и 72 %,
Moov 59 и 83 %, GOV.UK Pay 37 и 92 %, PayPal Payouts 34 и 72 %, Adyen
Transfers 36 и 80 %, Adyen Payout 23 и 46 %; из скачанных — ЮKassa 40 %,
Mollie 29 % (128 операций, 111 с ролью), Dwolla 42 %, Revolut 50 %.
Первая цифра ниже прежней после перехода на `payout_requisite`: реквизит,
для которого выражения платформы нет, теперь честно не считается покрытым.
Всё это одной командой: `./integrate --all --specs <каталог>`.
...
Где остался потолок и почему. Поля тел запросов: операция, которой контракт
не дал метода, теперь тоже получает свой метод и свой сборщик тела — на
Paystack это подняло измерение с 9 полей из 242 до 32, — но оставшиеся поля
либо необязательные без роли (по CLAUDE.md в payload не идут), либо
обязательные без роли (идут с TODO). Поднять эту цифру можно только новыми ролями полей
...
Промпт 10 закрыт: в каталоге десять спецификаций, три свои и шесть чужих
рядом с выданной, все десять проходят одной командой (`./integrate --all`), цифры и
...
печатает «игнорируется». Шаг 7 закрыт: покрытие печатается двумя цифрами, и
вычтенное из знаменателя названо поимённо. Шаг 8 закрыт: README переписан,
план выступления — в `docs/CHECKPOINT-3.md`.

Ночь на 6 сентября закрыла ещё четыре вещи, каждая нашлась прогоном на чужих
спеках, а не размышлением:

- **Рекурсивная схема больше не отвергает документ.** Цикл `$ref`
  размыкается, цепочка попадает в отчёт. Airwallex (4,3 МБ, 429 операций)
  проходит целиком.
- **Ключ операции стал один на весь конвейер.** У спецификации без
  `operationId` правило ошибки знало операцию как `post_payments`, а отчёт —
  как `POST /payments`; соединение молча не срабатывало. У ЮKassa коды
  ответов 65 → 100 %, общее покрытие 39 → 56 %.
- **Операция, которой контракт не дал метода, получает свой.** Вторая
  операция создания больше не теряется: у Paystack поля тел запросов 9 из 242
  → 32 из 242.
- **Три молчаливые догадки стали выбором с объяснением**: заголовок
...
`spec/fixtures/specs/real/README.md` прямо.
