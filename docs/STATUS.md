...
|---|---|---|
| CLI, справка, экран `analyze` | `locales/*/cli.yml` | ~90 |
| Ошибки загрузчика спецификации, формат места ошибки | `locales/*/spec_loader.yml` | 27 |
| Ошибки загрузчика справочников | `locales/*/rules.yml` | 108 |
| Предупреждения и обоснования: info, auth, operation | `locales/*/analyzers.yml` | 53 |
...
Всего ключей: 299 в `ru`, 293 в `en` (разница — формы множественного числа).
Overlay-фрагменты в обеих локалях побайтово одинаковы: это текст для машины.

Известные хвосты:

- `UNDERIVED = 'не выведено'` в `ir/units.rb`, `ir/idempotency.rb`,
  `ir/signature_profile.rb` — русский текст прямо в IR как обоснование по
  умолчанию. Эти типы пока не заполняет ни один анализатор; когда появятся
  анализаторы части 2 (промпт 5), обоснование должно приходить из них через
  `Texts.t`, а константа — уйти.
