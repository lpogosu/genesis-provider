...
для разработчиков: читается после `docs/PRINCIPLES.md`, обновляется в том же коммите, что
закрывает этап. Всё, что здесь написано, обязано воспроизводиться командой —
иначе это не состояние, а надежда.

Обновлено: 6 сентября 2026, после третьего чекпоинта. В каталоге десять
спецификаций: выданная, три свои (`cardpay`, `depositbank`, `broken`) и
шесть чужих настоящих (Adyen Payout, Adyen Transfers, PayPal Payouts,
Paystack, GOV.UK Pay, Moov); все десять проходят одной командой, результат
закоммичен в `examples/`. Сгенерированный класс исполняется стадией
проверки на своих же фикстурах; команда `diff` сравнивает две версии
спецификации через IR; покрытие печатается двумя цифрами.
...
SpecLoader → OverlayApplier → Analyzers + Matchers → IR → Generators → Validators → Reporter
  готов        готов          10 из 10     готов     готов   4 из 4     2 из 2   Summary + report.md
...
| Validators | готов: FixtureCheck (сверка фикстур со схемами спецификации) + ServiceCheck (прогон собранного класса на его же фикстурах, без сети) | `SpecGen::Validators.check(document:, profile:, fixtures_file:)` и `SpecGen::Validators.exercise(profile:, rules:, service_file:, fixtures_file:)` → `Result`; обе зовёт `Generators::Runner` после записи артефактов | `spec/unit/specgen/validators/` — 13 примеров на прогон, в том числе испорченный класс на каждую проверку; числа — в разделе 1 `output/report.md` и в строке CLI «Прогон собранного класса»; на десяти спецификациях каталога прогон не бросает исключений |
| Reporter | готов: `Summary` в консоли, `report.md` на диске и `DiffLines` — сравнение двух версий одной спецификации через IR, где у каждого отличия сказано, меняет ли оно сервис, документацию или только отчёт | `SpecGen::Reporter::Summary`, `SpecGen::Generators::ReportGenerator`, `SpecGen::Diff.call` и `SpecGen::Reporter::DiffLines` | `./integrate analyze --spec ... --explain`; `output/report.md`; `./integrate diff --old spec/fixtures/specs/novapay.yaml --new spec/fixtures/good/novapay_v2.yaml` |
...
| Web (API и фронт) | готов: пять маршрутов, один конверт ошибки, статика из `public/` | `SpecGen::Web.app` через `config.ru`; сервер — `ruby bin/serve --port 9292` | `spec/unit/specgen/web/` — 25 примеров; сводка отдаёт три уровня доверия в числах (`confidence`) и допущения раздела 9 (`assumptions`), тело запроса принимает `content`, `provider`, `locale`, `overlay`; живьём: `curl localhost:9292/api/health`, `/api/specs`, `POST /api/generate {"spec_id":"novapay"}`; содержимое артефактов из API байт-в-байт совпадает с тем, что пишет `./integrate --all` |

---

## Язык

Правило — в `docs/PRINCIPLES.md`, раздел «Язык»; термины — в `docs/GLOSSARY.md`.
...
```sh
./integrate --all                      # все десять спецификаций каталога, сводная таблица
...
двумя цифрами: у novapay 75 % (54 из 72 элементов) и 98 % в границах
контракта, у PayPal Payouts 34 и 72 %, Adyen Payout 23 и 46 %, Adyen
Transfers 36 и 80 %, Paystack 53 и 72 %, GOV.UK Pay 37 и 92 %. Дальше
...
одной выданной спеке. `spec/fixtures/specs/` держит десять файлов: выданная,
три свои и шесть чужих настоящих. Одна команда, одна таблица:
...
**Композиции схем разворачивает `SchemaNormalizer`, одна стадия на весь
конвейер.** `allOf` сливается рекурсивно (`required` объединяется, не
пересекается), `oneOf`/`anyOf` теряет ветку `null` и, если осталась одна
ветка, ею и становится, а если веток несколько — свойства всех попадают в
схему, обязательным не становится ни одно и каждое помнит свой вариант в
`Field#variant`. `discriminator` читается как условная обязательность
уровня 1 (`RequiredWhen` с origin `:discriminator`, уверенность 1.0) и
заодно даёт полю-дискриминатору `enum` из значений `mapping` — тот же вход
для ветки `case request_method`, что и `dependentRequired`. Свойство,
обязательное в каждой ветке, остаётся безусловно обязательным. `items` и
свойства-объекты регистрируются исходными узлами, а не свёрнутыми: о
композиции говорит та схема, в которую свойство превратилось, иначе одно
замечание печаталось бы дважды по одному адресу. Правила — публичный список
OpenAPI Normalizer из openapi-generator, подробности в `docs/IR.md`.
Замер до и после: Adyen Transfers 437 → 518 полей и 0 → 20 условий,
ЮKassa 991 → 1208 и 3 → 14, Checkout.com 10 564 → 12 766 и 6 → 80,
Lithic 2522 → 3194 и 1 → 45; на novapay, depositbank и broken не изменилось
ничего, у cardpay добавилась одна справка о свободной карте `metadata`.

**`enum` из чисел читается как текст.** Статусы и коды ошибок сравниваются
по записи (`ConstraintReader.enum_text`), поэтому провайдер, пронумеровавший
статусы (`status: 0 | 1 | 2`), больше не отдаёт пустую карту статусов молча;
незнакомое значение попадает в отчёт как `status_unmapped`. Само
ограничение в IR остаётся дословным: в код и в фикстуры уходит то, что
написал автор спецификации.
...
символами). Ничего про запас. Во второй части анализаторов добавлены три:
`signature_profile_conflict`, `webhook_event_undeclared`, `condition_unclear`;
в матчерах — два: `field_role_low_confidence`, `field_role_conflict`.
...
без группировки читать нельзя — задача отчёта, не матчеров.
...
целиком — в `templates/INTEGRATION.md.erb`, но текстом не в самом шаблоне:
заголовки, шапка и абзацы читаются из `generators.integration.heading.*` и
`generators.integration.prose.*`, поэтому `--locale en` переводит и их.
Так же устроен `templates/report.md.erb` (`generators.report.heading.*`,
`generators.report.prose.*`).
...
необязательные поля тел запросов без роли — по `docs/PRINCIPLES.md` они намеренно не
...
**Прогон собранного класса ничего не знает о провайдере и ничего не решает
за него.** `Validators::ServiceCheck` грузит записанный `service.rb` в
анонимный `Module` через `module_eval(source, path)`, положив туда подмену
базового класса платформы под именем из `rules/contract.yml`
(`Validators::Platform` собирает её из имён хелперов справочника, не из
литералов) и подменённый HTTP-клиент (`Validators::FakeClient` отвечает
телами из записанного `fixtures.json`). Объект операции собирает
`Validators::Payment` — обратный ход генерации: выражения контракта
(`operation.amount`, `operation.payout_requisite.dig(...)`,
`payload['raw_body']`) разбираются `Validators::Expression`, и значения
примера раскладываются по ним же. Модуль после прогона отбрасывается,
`ENV` базового адреса выставляется только на время загрузки и
восстанавливается.

**Проверка, чей итог зависел бы от часов, становится «не проверено», а не
«не прошло».** Профиль Standard Webhooks сверяет допуск метки времени по
`Time.now`, а фикстура подписана фиксированной меткой
(`SPECGEN_TIMESTAMP`): такой прогон дал бы разный `report.md` в разные дни и
сломал бы golden. То же правило закрывает `minimum: 0` (ниже нуля суммы не
бывает), заглушки OAuth2/HMAC (их значение считает метод, оставленный
человеку) и отсутствующее необязательное поле без роли — генератор не
отправляет его намеренно, и объявлять это ошибкой значило бы спорить с
собственной документацией. Отсутствие **обязательного** поля ошибкой
остаётся.

**Прогон нашёл три настоящих дефекта генератора** (6 сентября 2026), и это
и есть его оправдание: метод операции вне контракта без аргумента, но с
`operation` в теле (путь `{id}` без объявленного параметра),
`accept_response(operation, body)` в подтверждении без аргумента и
`IDEMPOTENCY_NAMESPACE`, которого не было, когда роль `idempotency_key`
досталась полю тела, а не заголовку. Все три чинились в презентерах
(`Service::Extras#params`, `#accepts?`, `Service::Constants#idempotency`),
байты golden novapay не изменились.
...
Хвосты, найденные прогонами на чужих спецификациях и оставленные с
записью здесь, а не молча.

- **Мок-провайдер как отдельный HTTP-сервер.** Эксперты сказали, что
  оценивают сгенерированный класс, а не живой трафик. Доказательство
  «запрос сформирован правильно» даёт стадия `Validators::ServiceCheck`:
  класс исполняется в песочнице с подменой платформы на своих же
  фикстурах, без сети. Отдельный сервер не добавил бы ничего, кроме сети.
- **Ограничение по статусу из прозы читается только у роли `cancel`.**
  Фраза «Confirmation is accepted only in status initiated or under_review»
  у `confirmDeposit` подходит под шаблон `only_in_status_en`, но
  `ConditionsAnalyzer` спрашивает об ограничении только операцию отмены, а
  вид условия в IR называется `cancel_status_restriction`. Расширение на
  `confirm` и `refund` — изменение модели, а не справочника.
- **`ref` в `broken.yaml` не получает ни одного кандидата роли.** Имя
  короче `min_length` 4 и не делит ни одного токена с синонимами
  `external_id` и `provider_operation_id`, поэтому это
  `required_field_role_unknown` — ровно тот случай «кандидатов нет вообще
  → пустое место с TODO», который третий уровень доверия и описывает.
- **Параметры получают роли полей тела по слабым сигналам.** У cardpay
  ...
  десять цифр. Предупреждение честное и роль ниже порога. Общее лекарство
  — запретить роли без `locations` в справочнике присваиваться параметру
  операции; это меняет поведение на всех спецификациях и требует замера,
  поэтому отложено.
- **`IR::Response` не помнит, был ли объявлен `content`.** Поэтому
  «`description` без `content`» и «`content` без `schema`» различаются
  эвристикой по коду ответа, а не структурно.
- **Прогон собранного класса** оставляет на чужих спецификациях по одной
  находке `failed`, и все они — настоящие пробелы, а не ложные тревоги:
  обязательное поле без выражения платформы (`cur` у broken,
  `amount.currency` у Adyen), пустой массив вместо элементов (PayPal
  `items`), роль `provider_operation_id` у поля `reference` в GOV.UK Pay.
  Проверка, чей итог зависел бы от `Time.now` (допуск метки времени
  Standard Webhooks), даёт `unchecked` с причиной: иначе `report.md` менялся
  бы по дате.
- **Stripe** (6,4 МБ, тысячи повторных `$ref`) не помещается в память после
  разворачивания; сообщение внятное, стектрейса нет. Снять предел можно,
  только если резолвер перестанет копировать разделяемые цели, а анализ
  пойдёт по документу лениво.
- **Раздел 9 `INTEGRATION.md` при `--locale en`** остаётся русским в
  строках допущений: они лежат данными в `rules/assumptions.yml` и
  `rules/contract.yml`, а не в локали. Перевод потребовал бы второй
  колонки в справочнике.

Проверено 6 сентября 2026: порядок предупреждений в `report.md` не зависит
от пути к файлу спецификации — прогон по `provider_api.yaml` и по
`spec/fixtures/specs/novapay.yaml` отличается только строками с именем
файла.
