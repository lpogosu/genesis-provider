...
Обновлено: 4 сентября 2026, после промпта 5 из `docs/PROMPTS.md`.
...
SpecLoader → OverlayApplier → Analyzers → IR → Generators → Validators → Reporter
  готов        нет            10 из 10    готов    нет         нет        Summary
```
...
Всего ключей: 431 в `ru`, 424 в `en` (разница — формы множественного числа).
...
createPayout»), девятью условиями взаимодействия, пятью операциями, восемью
схемами и четырнадцатью предупреждениями (0 ошибок, 5 предупреждений,
9 справок). Вторая добавляет обоснование под каждым выведенным значением и
Overlay-фрагмент под каждым исправимым предупреждением.
...
**Роли полей до матчеров не проставляются**, но для поиска поля суммы,
валюты, статуса, кода ошибки анализаторы используют точный словарный поиск
`RoleLookup#role_of` (уровень 2, `:registry`) и называют это в обосновании
(«имя `amount` совпало с синонимом роли amount в rules/roles.yml»). Код
ошибки узнаётся структурно: токен `code` под родителем-ошибкой; голое `code`
без такого родителя кодом ошибки не считается.
...
**Фикстурные справочники минимальны намеренно** (одни `names` у ролей), и
это значит, что структурные эвристики — код ошибки под родителем — на них
не проверить. Такие тесты (ErrorAnalyzer, OperationAnalyzer, SchemaAnalyzer)
гоняются на поставляемых `rules/`, и в шапке файла сказано почему.
...
Промпт 6 — матчеры полей: `NameMatcher`, `TypeMatcher`, `ConstraintMatcher`,
`StructureMatcher` с весами и порогом 0.6, проставляющие `Field#role` и
`Parameter#role`, которые сейчас `Derived.unknown` намеренно. Точный
словарный поиск `RoleLookup` — уже первый из этих матчеров, он должен стать
`NameMatcher`, а не остаться рядом с ним. Главное изменение после чекпоинта:
ниже порога роль всё равно присваивается лучшему кандидату с предупреждением,
`unknown` — только когда никто не проголосовал.

Сразу за ним промпт 7 (генераторы): ко второму чекпоинту 5 сентября эксперты
должны увидеть четыре файла из одной команды. Генерация не блокируется
никогда. Мок-провайдер (промпт 8) ушёл в бонус: эксперты оценивают
сгенерированный класс, а не живой трафик. Затем промпт 10 с настоящими
публичными спецификациями — заготовки уже лежат в `spec/fixtures/specs/real/`.
