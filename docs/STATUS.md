...
Промпт 6 из `docs/PROMPTS.md` — матчеры полей: `NameMatcher`, `TypeMatcher`,
`ConstraintMatcher`, `StructureMatcher` с весами и порогом 0.6, проставляющие
`Field#role` и `Parameter#role`, которые сейчас `Derived.unknown` намеренно.
Точный словарный поиск `RoleLookup` — уже первый из этих матчеров, он должен
стать `NameMatcher`, а не остаться рядом с ним. Сразу за ним промпт 7
(генераторы) и промпт 8 (мок-провайдер): на генерации стоит 25 баллов
экспертов, и ко второму чекпоинту эксперты должны увидеть один
сгенерированный сервис, а не десять анализаторов.
