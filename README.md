...
## CКРИНКАСТ: https://disk.yandex.ru/i/GXHy6zKyV658kg
...
Нейросетей в инструменте нет; каждый текст в артефактах имеет адрес в
репозитории.

| Что в артефакте | Откуда |
|---|---|
| Структура сервиса, YARD-комментарии к методам, постоянные пояснения | `templates/service.rb.erb` — разметка класса с комментариями прямо в шаблоне |
| Комментарии, зависящие от спецификации: «обязательно при type = sbp (намёк в описании 0.50)», TODO с кандидатами ролей, обоснование множителя | презентеры `lib/specgen/generators/service/` подставляют фразы из `locales/<код>/generators.yml`, данные — из промежуточной модели (источник, уверенность, обоснование каждого вывода) |
| Тексты `INTEGRATION.md` и `report.md` | `templates/*.md.erb` и презентеры `generators/integration/`, `generators/report/`; заголовки и проза — в `locales/<код>/generators.yml` |
| Предупреждения и обоснования | `locales/<код>/analyzers.yml`, `matchers.yml`, `schemas.yml` — по одному файлу на стадию |
| Допущения раздела 9 | `rules/assumptions.yml` и `rules/contract.yml` — данные, не docs |

Два языка: всё, что читает человек (консоль, документы, комментарии в
сгенерированном коде), лежит в `locales/ru/` и `locales/en/` и читается через
`Texts.t`; всё, что читает машина (идентификаторы, ключи YAML, названия
стандартов), остаётся английским. Новый язык — новый каталог, набор ключей
проверяет тест на паритет. Правила — [docs/PRINCIPLES.md](docs/PRINCIPLES.md), раздел «Язык».
...
ruby bin/ruby-share         # доля Ruby: 94.5 % (с ERB 95.3 %)
```
...
| Ruby больше 50 % кода | 94.5 % (с ERB 95.3 %) по правилам GitHub Linguist, проверяется `bin/ruby-share`; фронт — оболочка, вся логика в Ruby |
