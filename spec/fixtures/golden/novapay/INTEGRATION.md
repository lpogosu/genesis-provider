...
- №8. Выражения платформы — чтение полей операции (operation.amount и прочие) и предикат успешного результата базового класса — наш вывод; реального класса Operation не выдали. Поиска операции в хранилище и сохранения идентификатора провайдера в сервисе нет: по ответу экспертов это делает платформа снаружи. Источник: наше решение; эксперты 5 сентября 2026: «сохранение будет происходить вне сервиса провайдера, сервис провайдера должен сделать запрос к самому провайдеру, через абстрактный client и отдать ответ». Где влияет: build_payload, callback_target, accept_response.
