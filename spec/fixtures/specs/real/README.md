...
|---|---|---|---|---|---|
| `stripe_spec3.yaml` | [stripe/openapi](https://github.com/stripe/openapi), `openapi/spec3.yaml` | MIT | 6,4 МБ | размер | нагрузочный тест загрузчика; развёрнутый документ не помещается в память — единственная чужая спека, которую мы не берём |
...
| `airwallex.yaml` | [airwallex/airwallex-openapi](https://github.com/airwallex/airwallex-openapi), `openapi/client-api/latest/…yaml` | MIT | 4,3 МБ | размер | рекурсивная схема `IndustryCategoryV3Item`; из-за неё резолвер научился размыкать цикл, теперь проходит целиком — 429 операций |
...
инструмент понял и чего не понял; каждый пункт стал задачей для матчеров и справочников.
...
**Stripe (6,4 МБ, `tmp/`).** Циклический `$ref` (`file` → `file_link` →
`file`) больше не останавливает загрузку: с 6 сентября резолвер размыкает
цикл — первое вхождение разворачивается целиком, второе остаётся ссылкой
без полей, цепочка попадает в отчёт. Но Stripe всё равно не проходит, уже по
другой причине: развёрнутый документ не помещается в память. Тысячи
повторных ссылок на крупные схемы дают после разворачивания объём во много
раз больше исходных 6,4 МБ. Сообщение внятное и без стектрейса
(`cli.out_of_memory`), но это настоящий предел: чтобы его снять, резолвер
должен перестать копировать разделяемые цели, а анализ — ходить по документу
лениво. Единственная чужая спека из проверенных, которую мы не берём.
...
и `StructureMatcher`, второе — якорем основного ресурса и
группировкой отчёта, а не кодом под провайдера.
...
**Airwallex (4,3 МБ, tmp).** Был отказ на циклическом `$ref`
(`IndustryCategoryV3Item` ссылается сам на себя). Второй независимый случай
после Stripe и решил дело: рекурсивные схемы — обычная практика, а не
экзотика одного провайдера. С 6 сентября цикл размыкается, и Airwallex
проходит целиком: 429 операций, сгенерированный сервис на 14 678 строк,
`ruby -c` и RuboCop чистые. Разомкнутый цикл назван в отчёте отдельной
строкой — поля, которых в коде не будет, перечислены, а не потеряны молча.
