...
module SpecGen
  module Analyzers
    ...
    module ConstraintReader
      ...
      # Значение `enum` в том виде, в каком его сравнивают справочники.
      #
      # Статусы и коды ошибок сравниваются по тексту, а провайдер вправе
      # пронумеровать их (`status: 0 | 1 | 2`). Отбросить такой enum значило
      # бы отдать пустую карту статусов и не сказать почему; поэтому число
      # читается как его запись. Само ограничение в IR остаётся дословным:
      # в код и в фикстуры уходит то, что написал автор спецификации.
      #
      # @param value [Object] элемент `enum`
      # @return [String, nil] nil для null, объекта и списка — у них нет
      #   записи, по которой их можно узнать в справочнике
      def self.enum_text(value)
        case value
