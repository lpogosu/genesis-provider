...
module SpecGen
  ...
  # Стадия overlay: файл переопределений нельзя прочитать как документ
  # OpenAPI Overlay 1.0.0 — битый синтаксис, нет ключа `overlay` или
  # `actions`, действие без цели, цель написана синтаксисом JSONPath вне
  # поддерживаемого подмножества. Это ошибка ввода, но не спецификации:
  # overlay человек пишет и подключает руками, поэтому неприменимый файл
  # останавливает прогон, а не применяется наполовину. Цель, которой нет в
  # спецификации, ошибкой не является — это предупреждение
  # `overlay_target_missing`, генерацию не блокирует ничто.
  class OverlayError < Error; end
  ...
  # Стадия проверки: сверить сгенерированное со спецификацией нечем —
  # артефакт фикстур не читается как JSON. Несоответствие схеме сюда не
  # относится: оно не ошибка прогона, а находка отчёта, и генерацию не
  # блокирует ничто.
